...
// Sync: 15 LOW+HIGH pairs + HIGH gap → GapLow → LOW gap → data
// Sync pulses: 15 LOW + 15 HIGH = 30, then 1 HIGH gap pulse (GapLow) + 1 LOW gap pulse = 32
// Data: 64 bits × 2 = 128 transitions
// Total per burst: 32 sync + 128 data = 160
#define PC_ENCODER_BURSTS         3U
#define PC_ENCODER_SYNC_PAIRS     15U
#define PC_ENCODER_INTER_BURST_US 50000U
#define PC_ENCODER_UPLOAD_CAPACITY \
    ((PC_ENCODER_SYNC_PAIRS * 2U + 2U + PORSCHE_CAYENNE_BIT_COUNT * 2U) * PC_ENCODER_BURSTS + 5U)
_Static_assert(
    PC_ENCODER_UPLOAD_CAPACITY <= PP_SHARED_UPLOAD_CAPACITY,
    "PC_ENCODER_UPLOAD_CAPACITY exceeds shared upload slab");

typedef struct SubGhzProtocolEncoderPorscheCayenne {
...
static void subghz_protocol_encoder_porsche_cayenne_get_upload(
    SubGhzProtocolEncoderPorscheCayenne* instance) {
    furi_check(instance);

    // Compute the 8-byte frame.
    // generic.cnt is the 1-indexed counter found by brute-force scan;
    // compute_frame was called with (try_cnt - 1), so pass (cnt - 1).
    uint8_t pkt[8];
    uint16_t enc_cnt = (instance->cnt > 0) ? (uint16_t)(instance->cnt - 1) : 0;
    porsche_cayenne_compute_frame(
        instance->serial & 0xFFFFFFUL, instance->btn, enc_cnt, 0x04U, pkt);
...
#define ADD_LEVEL(lvl, dur) \
    index = pp_emit_merge(instance->encoder.upload, index, PC_ENCODER_UPLOAD_CAPACITY, (lvl), (dur))

    for(uint8_t burst = 0; burst < PC_ENCODER_BURSTS; burst++) {
        // Sync: LOW PC_TE_SYNC, HIGH PC_TE_SYNC … ×15 pairs
        // Starting with LOW enters Reset→Sync at sync_count=1.
        for(uint8_t s = 0; s < PC_ENCODER_SYNC_PAIRS; s++) {
            ADD_LEVEL(false, PC_TE_SYNC);
            ADD_LEVEL(true, PC_TE_SYNC);
        }
        // After 15 LOW pulses (sync_count=15), emit HIGH gap → GapLow state
        ADD_LEVEL(true, PC_TE_GAP);
        // Then LOW gap → triggers GapLow → Data state
        ADD_LEVEL(false, PC_TE_GAP);

        // Data: 64 bits MSB first
        // bit=0: (short_low, long_high), bit=1: (long_low, short_high)
        for(uint8_t byte_i = 0; byte_i < 8U; byte_i++) {
            for(int8_t bit_i = 7; bit_i >= 0; bit_i--) {
                bool bit = (pkt[byte_i] >> bit_i) & 1U;
                if(bit) {
        ...
        if(burst < PC_ENCODER_BURSTS - 1U) {
            ADD_LEVEL(false, PC_ENCODER_INTER_BURST_US);
        }
    }
...
void* subghz_protocol_encoder_porsche_cayenne_alloc(SubGhzEnvironment* environment) {
    ...
    pp_encoder_buffer_ensure(instance, PC_ENCODER_UPLOAD_CAPACITY);
...
SubGhzProtocolStatus subghz_protocol_encoder_porsche_cayenne_deserialize(
    ...
    do {
        ...
        instance->serial = serial & 0xFFFFFFUL;
        ...
        subghz_protocol_encoder_porsche_cayenne_get_upload(instance);
