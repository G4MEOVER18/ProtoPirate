...
void protopirate_settings_set_defaults(ProtoPirateSettings* settings) {
    ...
    settings->history_auto_purge = false;
}
...
void protopirate_settings_load(ProtoPirateSettings* settings) {
    ...
    do {
        ...
        uint32_t hist_limit_temp = 10;
        if(!flipper_format_read_uint32(ff, "HistoryLimit", &hist_limit_temp, 1)) {
            hist_limit_temp = 10;
        }
        if(hist_limit_temp != 5 && hist_limit_temp != 10 &&
           hist_limit_temp != 20 && hist_limit_temp != 50) {
            hist_limit_temp = 10;
        }
        settings->history_limit = (uint8_t)hist_limit_temp;

        uint32_t hist_purge_temp = 0;
        if(!flipper_format_read_uint32(ff, "HistoryAutoPurge", &hist_purge_temp, 1)) {
            hist_purge_temp = 0;
        }
        settings->history_auto_purge = (hist_purge_temp == 1);
...
void protopirate_settings_save(ProtoPirateSettings* settings) {
    ...
    do {
        ...
        uint32_t hist_limit_save = settings->history_limit;
        if(!flipper_format_write_uint32(ff, "HistoryLimit", &hist_limit_save, 1)) {
            FURI_LOG_E(TAG, "Failed to write history limit");
            break;
        }

        uint32_t hist_purge_save = settings->history_auto_purge ? 1 : 0;
        if(!flipper_format_write_uint32(ff, "HistoryAutoPurge", &hist_purge_save, 1)) {
            FURI_LOG_E(TAG, "Failed to write history auto-purge");
            break;
        }
