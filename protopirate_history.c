...
struct ProtoPirateHistory {
    ...
    uint8_t history_limit;
    bool history_auto_purge;
};
...
ProtoPirateHistory* protopirate_history_alloc(void) {
    ...
    instance->loaded_ff = NULL;
    instance->loaded_idx = -1;
    instance->history_limit = PROTOPIRATE_HISTORY_MAX;
    instance->history_auto_purge = false;
...
void protopirate_history_format_status_text(
    ...
    if(n >= instance->history_limit) {
        snprintf(output, output_size, "FULL");
    } else {
        snprintf(output, output_size, "%u/%u", n, instance->history_limit);
    }
}
...
bool protopirate_history_add_to_history(
    ...
    if(ProtoPirateHistoryItemArray_size(instance->data) >= instance->history_limit) {
        if(instance->history_auto_purge) {
            protopirate_history_delete_item(instance, 0);
        } else {
            return false;
        }
    }
...
void protopirate_history_set_limit(ProtoPirateHistory* instance, uint8_t limit) {
    furi_check(instance);
    instance->history_limit = (limit > 0) ? limit : PROTOPIRATE_HISTORY_MAX;
}

void protopirate_history_set_auto_purge(ProtoPirateHistory* instance, bool auto_purge) {
    furi_check(instance);
    instance->history_auto_purge = auto_purge;
}
