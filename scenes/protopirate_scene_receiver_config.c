...
enum ProtoPirateSettingIndex {
    ...
    ProtoPirateSettingIndexHistoryLimit,
    ProtoPirateSettingIndexHistoryPurge,
    ProtoPirateSettingIndexClearHistory,
...
#define HISTORY_LIMIT_COUNT 4
static const char* const history_limit_text[HISTORY_LIMIT_COUNT] = {"5", "10", "20", "50"};
static const uint8_t history_limit_value[HISTORY_LIMIT_COUNT] = {5, 10, 20, 50};

#define HISTORY_PURGE_COUNT 2
static const char* const history_purge_text[HISTORY_PURGE_COUNT] = {"OFF", "ON"};
...
static void protopirate_scene_receiver_config_set_history_limit(VariableItem* item) {
    ProtoPirateApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
    app->settings.history_limit = history_limit_value[index];
    variable_item_set_current_value_text(item, history_limit_text[index]);
    protopirate_history_set_limit(app->txrx->history, app->settings.history_limit);
    protopirate_settings_save(&app->settings);
}

static void protopirate_scene_receiver_config_set_history_purge(VariableItem* item) {
    ProtoPirateApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
    app->settings.history_auto_purge = (index == 1);
    variable_item_set_current_value_text(item, history_purge_text[index]);
    protopirate_history_set_auto_purge(app->txrx->history, app->settings.history_auto_purge);
    protopirate_settings_save(&app->settings);
}
...
static void
    ...
    } else if(index == ProtoPirateSettingIndexClearHistory) {
        view_dispatcher_send_custom_event(
            app->view_dispatcher, ProtoPirateCustomEventSceneSettingClearHistory);
    }
}
...
void protopirate_scene_receiver_config_on_enter(void* context) {
    ...
    item = variable_item_list_add(
        ...
        "History Limit:",
        HISTORY_LIMIT_COUNT,
        protopirate_scene_receiver_config_set_history_limit,
        app);
    {
        uint8_t limit_idx = 1;
        for(uint8_t i = 0; i < HISTORY_LIMIT_COUNT; i++) {
            if(history_limit_value[i] == app->settings.history_limit) {
                limit_idx = i;
                break;
            }
        }
        variable_item_set_current_value_index(item, limit_idx);
        variable_item_set_current_value_text(item, history_limit_text[limit_idx]);
    }
    ...
    item = variable_item_list_add(
        ...
        "Auto-Purge:",
        HISTORY_PURGE_COUNT,
        protopirate_scene_receiver_config_set_history_purge,
        app);
    {
        uint8_t purge_idx = app->settings.history_auto_purge ? 1 : 0;
        variable_item_set_current_value_index(item, purge_idx);
        variable_item_set_current_value_text(item, history_purge_text[purge_idx]);
    }

    // Clear History
    {
        static char clear_label[32];
        snprintf(
            clear_label,
            sizeof(clear_label),
            "Clear History [%u]",
            protopirate_history_get_item(app->txrx->history));
        variable_item_list_add(app->variable_item_list, clear_label, 1, NULL, app);
    }
...
bool protopirate_scene_receiver_config_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == ProtoPirateCustomEventSceneSettingLock) {
            ...
            scene_manager_previous_scene(app->scene_manager);
            consumed = true;
        } else if(event.event == ProtoPirateCustomEventSceneSettingClearHistory) {
            if(app->txrx->history) {
