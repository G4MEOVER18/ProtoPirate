...
void protopirate_scene_sub_decode_on_enter(void* context) {
    ...
    g_decode_ctx->raw_reader = NULL;
    g_decode_ctx->decode_start_tick = furi_get_tick();
...
bool protopirate_scene_sub_decode_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeTick) {
        ...
        if((ctx->state == DecodeStateDecodingRaw || ctx->state == DecodeStateStartingWorker) &&
           (furi_get_tick() - ctx->decode_start_tick > SUB_DECODE_TIMEOUT_MS)) {
            furi_string_set(ctx->result, "Decode timeout");
            furi_string_set(ctx->error_info, "File too large or complex");
